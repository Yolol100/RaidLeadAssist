...
for _, d in ipairs({"normal","heroic","mythic"}) do
    ...
    assert(p.callsByKey.maelstrom.warning == "Maelstrom: assigned Poppers trigger Cysts.")
    assert(p.callsByKey.apex.warning == "Mutilate: assigned group soak frontal.")
    assert(p.callsByKey.dig_in.warning == "Dig In: use damage cooldowns.")
end
...
print("ok - Sszorak plan, assignments and shared calls stay aligned")
