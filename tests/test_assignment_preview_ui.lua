...
assert(contains(assignmentFrame, 'self.status:SetPoint("BOTTOMLEFT", 16, 61)')
    and contains(assignmentFrame, 'self.status:SetPoint("RIGHT", -16, 0)')
    and contains(assignmentFrame, 'self.required:SetPoint("BOTTOMLEFT", 16, 49)')
    and contains(assignmentFrame, 'self.required:SetPoint("RIGHT", -16, 0)'),
    "AssignmentFrame must own the compact full-width footer feedback layout above the action row")
