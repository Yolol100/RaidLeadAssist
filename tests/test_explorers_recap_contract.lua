...
assert(contains(normal, "Fish appears"))
assert(contains(normal, "Three players marked"))
assert(contains(normal, "Fire clears with Frost"))
assert(contains(normal, "use a mushroom"))
assert(contains(normal, "Icebound Flames starts"))
assert(not contains(normal, "Nama > Iku > Gebbo"), "raidleader fish order belongs in pre-pull setup")

local heroic = plan("heroic")
assert(contains(heroic, "Keep Nama away"))
assert(contains(heroic, "Spreading fire appears"))
assert(not contains(heroic, "Fish appears"), "Heroic should only contain changes from Normal")
