...
for _, difficulty in ipairs({"normal","heroic","mythic"}) do
    ...
    assert(sent.callsByKey.stasis.warning == "Stasis: pair toxins to exactly 4.")
...
assert(has(sentNormal, "heal the weaker boss"))
...
for _, difficulty in ipairs({"normal","heroic","mythic"}) do
    ...
    assert(nek.callsByKey.phase2.warning == "Phase 2: Bloodlust; burn before full energy.")
end
...
assert(has(nekHeroic, "everyone else stays out"))
