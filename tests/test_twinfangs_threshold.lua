...
assert(string.find(text, "THREE FRESH 3+ TEAMS", 1, true), "Heroic strategy must state the stable Ravenous Feast minimum and fresh-team plan")
...
assert(heroic.callsByKey.stone == nil, "Stone Breaker live role execution must stay bossmod-owned")
assert(string.find(text, "AT LEAST ONE PLAYER MUST BE HIT BY EACH SLAM", 1, true), "the plan must still state Stone Breaker's raid-failure condition")
assert(not string.find(text, "KILLS YOU AT", 1, true), "volatile Eternal Venom death thresholds must not be hard-coded")

print("ok - Twin Fangs uses stable venom, fresh Feast teams, bossmod role ownership and joint-finish invariants")
