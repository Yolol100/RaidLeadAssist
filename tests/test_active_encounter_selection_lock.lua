...
ns:RegisterModule("Encounters.Registry", {
    Get=function(_, key) if key == "nekzali" or key == "sentinels" then return {key=key} end end,
    GetProfile=function(_, key, difficulty) if (key == "nekzali" or key == "sentinels") and difficulty then return {calls={}, callsByKey={}} end end,
...
ns:RegisterModule("Services.TimelineService", { SetEncounter=function(_, key, preserve) timelineCalls[#timelineCalls+1]={key=key,preserve=preserve} end })
...
assert(App:SelectDifficulty("normal", false) == false, "Normal must be unsupported even before the encounter lock is considered")
assert(#timelineCalls == 0)
assert(App:SelectBoss("sentinels", false) == false)
assert(App.activeBossKey == "nekzali" and selectedUIBoss == nil and #timelineCalls == 0)

assert(App:SelectDifficulty("mythic", true) == true,
    "automatic supported difficulty selection must bypass the manual encounter lock")
assert(selectedDifficulty == "mythic" and App.activeDifficultyKey == "mythic")
...
print("ok - active encounter selection locks supported Heroic/Mythic profiles and rejects Normal")
