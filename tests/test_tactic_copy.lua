...
assert(contains(planText("vashnik", "normal"), "SWAP AFTER EACH DRIPPING FANGS"))
assert(contains(planText("vashnik", "heroic"), "SWAP AFTER EACH DRIPPING FANGS"))
assert(contains(planText("vashnik", "mythic"), "SWAP AFTER EACH DRIPPING FANGS"))
...
assert(vashnikNormal.callsByKey.tankswap.prepareSeconds == 5)
