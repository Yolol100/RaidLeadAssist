...
for _, difficultyKey in ipairs(Constants.DIFFICULTY_ORDER) do
    ...
    assert(Registry:GetProfile("sentinels", difficultyKey).callsByKey.tankswap)
end
...
assert(contains(sentinelsNormal.callsByKey.miasma.warning, "RAID SOAK"))
...
assert(contains(vashnikMythic.callsByKey.totems.warning, "FROTH WAVES"))
assert(contains(vashnikMythic.callsByKey.froth.warning, "TUMORS"))
for _, difficultyKey in ipairs(Constants.DIFFICULTY_ORDER) do
    local profile = Registry:GetProfile("vashnik", difficultyKey)
    assert(profile.callsByKey.siphon)
    assert(profile.callsByKey.exploding)
    assert(profile.callsByKey.stygian)
    assert(profile.callsByKey.tankswap)
end
assert(contains(vashnikNormal.callsByKey.siphon.warning, "HEAL ABSORB"))
assert(contains(vashnikNormal.callsByKey.exploding.warning, "FAR OUT"))
assert(contains(vashnikNormal.callsByKey.stygian.warning, "KEEP MOVING"))
...
for _, profile in ipairs({ nekzaliNormal, nekzaliHeroic, nekzaliMythic }) do
    ...
    assert(profile.callsByKey.pyre.warning == "GROUP 1 + 2 SOAK")
    assert(profile.callsByKey.phase2.warning == "PHASE 2 > BLOODLUST > BURN BOSS")
end
assert(nekzaliNormal.callsByKey.flame.warning == "GROUP 3 + 4 SPREAD OUT")
assert(nekzaliHeroic.callsByKey.flame.warning == "GROUP 3 + 4 BURN ADS")
