...
local first = Assignments:BuildCallWarning("GREEN MUTILATE > SOAK", "sszorak", "heroic", "apex")
...
local second = Assignments:BuildCallWarning("GREEN MUTILATE > SOAK", "sszorak", "heroic", "apex")
assert(second:find("GROUP 2: Groups 3+4", 1, true), "second Mutilate call must use configured Soak Group 2")
Assignments:AdvanceCall("sszorak", "heroic", "apex")

local third = Assignments:BuildCallWarning("GREEN MUTILATE > SOAK", "sszorak", "heroic", "apex")
assert(third:find("GROUP 1: Groups 1+2", 1, true), "Mutilate group rotation must loop back to Group 1")

Assignments:ResetRuntime()
local reset = Assignments:BuildCallWarning("GREEN MUTILATE > SOAK", "sszorak", "heroic", "apex")
assert(reset:find("GROUP 1: Groups 1+2", 1, true), "Mutilate rotation must reset to Group 1 between pulls")

print("ok - Sszorak Mutilate uses two group-level assignments and rotates Group 1 > Group 2")
