...
The active timeline label shows whether the selected representation currently comes from DBM, BigWigs, Blizzard, or the BigWigs/Blizzard bridge. Settings and `/rla provider` distinguish the bossmod core from the The Venomous Abyss/Midnight raid content pack, report whether DBM currently suppresses Blizzard-derived timers, and summarize the currently active timer paths and how many of those timers match an RLA call. `/rla doctor` also prints the DBM 12.1.3 and BigWigs v419.2 callback-contract baselines used by this release so a version drift is visible next to the detected addon versions without hard-blocking newer bossmods.
...
Automatic timing is independently controllable with the `AUTO ON/OFF` Settings button or `/rla timing on|off`. Turning automatic timing off idles timer-driven PREPARE/PRESS state and TTS while leaving the raid plan, manual call buttons, assignments, and Raid Warning text fully usable. Encounter safety remains a separate gate: turning the user switch on can never override an unknown boss, unsupported difficulty, or other fail-closed encounter state.

`/rla doctor` is a read-only pre-pull readiness check. It reports SavedVariables schema compatibility, selected boss/difficulty, raid/encounter verification, Raid Warning permission, automatic-timing user/runtime state, TTS or sound-fallback readiness, bossmod/core-pack versions, the callback-contract baseline, currently active/matched timer paths, and the encounter strategy validation note. `/rla provider` prints the provider diagnostics plus the active timer-path subset.
