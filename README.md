...
The provider adapters and TimelineService fail closed on Midnight secret values. Secret timer durations, update values, identities, metadata, encounter IDs, and pause state are not retained or used for call decisions. Blizzard Encounter Timeline entries whose source is not the live encounter source are ignored, so Edit Mode/test timeline events are not treated as live raid mechanics.

Ula'tek remains manual-timing only until its RLA call set has stable public timer identities in the supported live bossmod/runtime paths. Current BigWigs still exposes Ula'tek primarily through the Blizzard Encounter Timeline bridge and current DBM has only partial timer coverage, so RLA does not infer a complete automatic call schedule from either provider. Its current difficulty plans are Journal-derived and intentionally do not claim live raid validation. Where pre-release guide text and newer Encounter Journal or bossmod data disagree, Raid Lead Assist keeps automatic timing disabled and avoids inventing an exact mechanic identity.
