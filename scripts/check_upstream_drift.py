#!/usr/bin/env python3
"""Validate and optionally check RaidLeadAssist's upstream provider baselines."""
    ...
    for provider in providers:
        ...
        for watch in watches:
            if not isinstance(watch, dict):
                fail(f"{name}: watch entries must be objects")
            path = watch.get("path")
            ref = watch.get("ref")
            blob_sha = watch.get("blobSha")
            if not isinstance(path, str) or not path or path.startswith("/") or ".." in Path(path).parts:
                fail(f"{name}: invalid watch path {path!r}")
            if not isinstance(ref, str) or not ref.strip():
                fail(f"{name}: watch ref is required for {path}")
            if not isinstance(blob_sha, str) or not SHA_RE.fullmatch(blob_sha):
                fail(f"{name}: invalid blobSha for {path}")
            key = (ref, path)
            if key in seen_paths:
                fail(f"{name}: duplicate watch path {ref}:{path}")
            seen_paths.add(key)
...
def check_online(data: dict) -> list[str]:
    ...
    for provider in data["providers"]:
        ...
        if latest_name != provider["releaseTag"]:
            drift.append(
                f"{name}: latest release changed: expected {provider['releaseTag']}, got {latest_name}"
            )
        else:
        ...
        for watch in provider["watchPaths"]:
            ...
            if actual_sha != watch["blobSha"]:
                drift.append(
                    f"{name}: watched upstream file changed: {watch['ref']}:{watch['path']} "
...
def main() -> int:
    ...
    print(f"ok - upstream baseline schema valid ({len(data['providers'])} providers, reviewed {data['reviewedAt']})")
    if not args.online:
        return 0
    ...
    print("ok - no watched DBM/BigWigs release or source drift detected")
    return 0
